...
echo "[sweep] env=$ENV t_max=$T_MAX seeds=$SEEDS algos=${ALGOS[*]} sharing=${SHARINGS[*]}"
...
for seed in $SEEDS; do
  for algo in "${ALGOS[@]}"; do
    for sharing in "${SHARINGS[@]}"; do
      i=$((i+1))
      out="results/$ENV/$algo/$sharing/seed$seed"
      ...
      "$VENV_PY" exp_train.py --algo "$algo" --sharing "$sharing" --env "$ENV" --seed "$seed" --t_max "$T_MAX" \
